    ...
    public void update() {
        ...
        if (!attachedTile.hasCapability(EmbersCapabilities.EMBER_CAPABILITY, facing)
            ...
            if (cap != null && cap.canReceive() && cap.getEnergyStored() < cap.getMaxEnergyStored()) {
                int added = cap.receiveEnergy((int) Math.min(TRANSFER_SPEED * Options.mulitiplier, capability.getEmber() * Options.mulitiplier), true);
                if (added > 0) {
                    cap.receiveEnergy((int) Math.min(TRANSFER_SPEED * Options.mulitiplier, capability.getEmber() * Options.mulitiplier), false);
